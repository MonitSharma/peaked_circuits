#!/usr/bin/env python3
"""Submit the independent third P6 100-shot discovery batch."""
...
OUT = ROOT / "hardware_campaign/p6_batch_003/job_record.json"
JOB_NAME = "p6_physical_discovery_100_batch3"
SHOTS = 100
MAX_COST = 900.0
...
def main() -> int:
    ...
    project = qnx.projects.get_or_create(
        ...
        description="P6 independent third discovery batch",
    )
    artifact = qnx.qir.upload(
        qir=BITCODE.read_bytes(),
        name=JOB_NAME,
        project=project,
        description="Pinned P6 QIR; independent third discovery batch; no candidate target",
    )
    job = qnx.start_execute_job(
        ...
        project=project,
        name=JOB_NAME,
        description="Independent P6 discovery batch 3; analyze separately before pooling",
    )
    payload = {
        "status": "SUBMITTED",
        "stage": "independent_discovery_batch_3",
