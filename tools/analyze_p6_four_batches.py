#!/usr/bin/env python3
"""Analyze P6 discovery batches 1-4 independently, then pool 400 frames."""
...
OUT = ROOT / "results/hardware/p6_helios_four_batch_analysis_20260907"
QUBITS = 62
...
def decode(shots: list[str]) -> dict[str, object]:
    ...
    candidates = [most_frequent_string(counts), bitwise_majority_string(counts), weighted_observed_medoid(counts), cluster_consensus(counts)]
    return {
...
def bit_stability(shots: list[str]) -> dict[str, object]:
    ...
    return {"threshold": "p1 <= 0.25 or p1 >= 0.75", "stable_position_count": len(stable), "stable_positions_zero_based": stable, "bitwise_ensemble_candidate": "".join("1" if p > 0.5 else "0" for p in p1), "p1_by_position": p1}
...
def main() -> int:
    ...
    for path in RAW:
        ...
        batches.append(shots[:100])
        framing.append(info)
    decoded = [decode(batch) for batch in batches]
    pooled = [shot for batch in batches for shot in batch]
    pooled_decoded = decode(pooled)
    modes = [get_candidate(d, "most_frequent") for d in decoded]
    payload = {
        "schema_version": "p6-four-discovery-batches-v1",
        "target_blind": True,
        "provider_submission_performed": False,
        "framing": framing,
        "batches": decoded,
        "pooled_400": pooled_decoded,
        "mode_comparison": {"batch_modes": modes, "pairwise_hamming": [[hamming(a, b) for b in modes] for a in modes], "pooled_mode": get_candidate(pooled_decoded, "most_frequent")},
        "bit_stability": bit_stability(pooled),
        "interpretation": {"pooled_unique_strings": pooled_decoded["unique_strings"], "pooled_mode_count": pooled_decoded["top_counts"][0]["count"], "decision": "NO_REPRODUCIBLE_FULL_CANDIDATE", "reason": "The four batches do not share a recurring mode and the pooled distribution remains diffuse; pooling does not establish a complete P6 answer."},
    }
