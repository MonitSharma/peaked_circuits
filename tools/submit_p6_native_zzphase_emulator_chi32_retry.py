#!/usr/bin/env python3
"""Submit a confirmation one-shot chi=32 Helios-1E emulator probe."""
...
OUT = ROOT / "results/hardware/p6_optimized_compile_20260907/native_zzphase_helios1e_chi32_retry.json"
JOB_NAME = "p6_native_zzphase_helios1e_chi32_retry_20260907"
...
def main() -> int:
    ...
    artifact = qnx.qir.upload(
        ...
        name=JOB_NAME,
        project=project,
        description="Confirmation one-shot Helios-1E MPS probe; chi=32",
    )
    ...
    job = qnx.start_execute_job(
        ...
        project=project,
        name=JOB_NAME,
        description="Confirmation P6 native-ZZPhase one-shot chi=32 emulator probe; not hardware evidence",
    )
    payload = {
        "status": "SUBMITTED",
