#!/usr/bin/env python3
"""Submit one controlled chi=128 Helios-1E MPS probe."""
...
OUT = ROOT / "results/emulator/p6_chi128_probe_20260907/submission.json"
JOB_NAME = "p6_chi128_probe_20260907"
...
def main() -> int:
    ...
    artifact = qnx.qir.upload(
        ...
        name=JOB_NAME,
        project=project,
        description="Controlled one-shot chi=128 Helios-1E MPS probe",
    )
    ...
    job = qnx.start_execute_job(
        ...
        project=project,
        name=JOB_NAME,
        description="P6 chi=128 MPS probe; emulator only, not hardware evidence",
    )
    payload = {
        "status": "SUBMITTED",
