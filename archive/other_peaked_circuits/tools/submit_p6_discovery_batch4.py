#!/usr/bin/env python3
"""Submit the independent fourth P6 100-shot discovery batch."""
...
OUT = ROOT / "hardware_campaign/p6_batch_004/job_record.json"
JOB_NAME = "p6_physical_discovery_100_batch4"
SHOTS = 100
MAX_COST = 900.0
...
def main() -> int:
    ...
    project = qnx.projects.get_or_create(
        ...
        description="P6 independent fourth discovery batch",
    )
    artifact = qnx.qir.upload(
        qir=BITCODE.read_bytes(),
        name=JOB_NAME,
        project=project,
        description="Pinned P6 QIR; independent fourth discovery batch; no candidate target",
    )
    job = qnx.start_execute_job(
        ...
        project=project,
        name=JOB_NAME,
        description="Independent P6 discovery batch 4; analyze separately before pooling",
    )
    payload = {
        "status": "SUBMITTED",
        "stage": "independent_discovery_batch_4",
